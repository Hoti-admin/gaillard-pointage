...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase.from("profiles").select("role,is_active,full_name").eq("user_id", session.user.id).single();
      if (error) setMsg("Erreur profil: " + error.message);
      else setProfile((data as any) ?? null);
    })();
  }, [session?.user?.id]);
  ...
  return (
    <main style={S.page}>
      <div style={S.container}>
        ...
        <div style={S.grid}>
          <a href="/admin/review" style={S.tile}>
            <div style={S.tileTitle}>🧾 Contrôle bordereau</div>
            <div style={S.tileSub}>Voir / corriger / valider avant export</div>
          </a>
