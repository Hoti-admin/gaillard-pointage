...
const S = {
  ...
  msg: { marginTop: 12, padding: "10px 12px", borderRadius: 12, border: `1px solid ${THEME.border}`, background: awareBg(), fontWeight: 800 },
...
export default function AdminEmployeesPage() {
  ...
  async function createEmployee() {
    ...
    setMsg(temp ? `✅ Employé créé. Mot de passe provisoire: ${temp}` : "✅ Employé créé.");
  ...
  async function toggleActive(row: EmpRow) {
    setLoading(true);
    setMsg("");
    const { error } = await supabase.from("profiles").update({ is_active: !row.is_active }).eq("user_id", row.user_id);
    setLoading(false);
    if (error) {
  ...
  async function resetPassword(row: EmpRow) {
    if (!session?.access_token) return;
    const ok = window.confirm(`RESET mot de passe pour ${row.full_name || row.email} ? (nouveau Gaillard-YYYY@XXXX)`);
    if (!ok) return;
