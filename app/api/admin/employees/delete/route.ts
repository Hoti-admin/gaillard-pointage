...
export async function POST(req: Request) {
  try {
    ...
    // 🔎 Lire le profil de la personne à supprimer
    ...
    // ✅ Sécurité: empêcher de supprimer le dernier admin ACTIF
    if (targetRole === "admin" && targetActive) {
      ...
      if (adminsActifs <= 1) {
        return NextResponse.json(
          { error: "Impossible de supprimer le dernier admin actif." },
          { status: 400 }
        );
      }
    }

    // 🧹 Supprimer d'abord le profil (optionnel, mais ok)
    await supabaseAdmin.from("profiles").delete().eq("user_id", user_id);

    // 🗑️ Supprimer le user auth (cascade sur tes tables FK auth.users)
