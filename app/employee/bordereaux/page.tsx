...
export default function EmployeeBordereauxPage() {
  ...
  async function fetchStatuses(yearToLoad: number) {
    try {
      ...
      const res = await fetch(
        `/api/employee/timesheets/month-status?year=${yearToLoad}`,
        {
      ...
      const approvedForYear = nextRows
        .filter(
          (r) =>
            r.status === "approved" && r.month.startsWith(`${yearToLoad}-`)
        )
        .map((r) => r.month)
        .sort();
      ...
      if (approvedForYear.length > 0) {
        ...
        const fallback = `${yearToLoad}-${pad2(
          yearToLoad === now.getFullYear() ? now.getMonth() + 1 : 12
        )}`;
        setSelectedMonth(fallback);
      }
    } catch (error) {
      console.error(error);
      setMsg("Erreur inattendue lors du chargement des bordereaux.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function downloadAuthed(url: string, filename: string) {
    try {
      ...
      console.error(error);
      setMsg("Erreur inattendue pendant le téléchargement.");
    }
  }

  async function exportSelectedPDF() {
    await downloadAuthed(
      `/api/employee/export/pdf?month=${encodeURIComponent(selectedMonth)}`,
      `Bordereau_${selectedMonth}.pdf`
    );
  }

  async function exportSelectedXLSX() {
    await downloadAuthed(
      `/api/employee/export/xlsx?month=${encodeURIComponent(selectedMonth)}`,
      `Bordereau_${selectedMonth}.xlsx`
    );
  }

  async function exportRowPDF(month: string) {
    await downloadAuthed(
      `/api/employee/export/pdf?month=${encodeURIComponent(month)}`,
  ...
  async function exportRowXLSX(month: string) {
    await downloadAuthed(
      `/api/employee/export/xlsx?month=${encodeURIComponent(month)}`,
  ...
  const approvedCount = approvedRows.filter((r) =>
    r.month.startsWith(`${selectedYear}-`)
  ).length;

  const pendingCount = monthOptions.filter(
    (m) => m.status !== "approved"
  ).length;
      ...
      <div style={{ ...containerStyle, padding: isMobile ? 14 : 20 }}>
        ...
        >
          ...
          >
            <img
              ...
              alt="Gaillard"
              style={{ ...logoStyle, width: isMobile ? 96 : 180 }}
            />
            <div>
              <h1 style={{ ...titleStyle, fontSize: isMobile ? 28 : 32 }}>
                Mes bordereaux validés
              </h1>
              <div style={subStyle}>
                Par mois et par année, avec accès rapide au PDF et à l’Excel.
              </div>
            </div>
          </div>
        ...
        >
          <div style={cardShell}>
            ...
            >
              {approvedCount > 0
                ? `${approvedCount} bordereau${
                    approvedCount > 1 ? "x" : ""
                  } validé${approvedCount > 1 ? "s" : ""}`
                : "Aucun bordereau validé"}
            </div>

            <div style={{ color: THEME.sub, marginTop: 8, fontWeight: 700 }}>
              Sélectionne une année pour voir l’historique disponible et
              télécharge directement le mois voulu.
            </div>
            ...
            >
              <div style={statsPill(THEME.green)}>
                {approvedCount} validé{approvedCount > 1 ? "s" : ""}
              </div>
              <div style={statsPill(THEME.amber)}>
                {pendingCount} en attente
              </div>
              <div style={statsPill(THEME.blue)}>
                Année {selectedYear}
              </div>
            </div>
          </div>
          ...
          <div style={cardShell}>
            ...
            >
              <span style={statusBadge(selectedStatus)}>
                {selectedStatus === "approved" ? "● Validé" : "● En attente"}
              </span>
              <span style={{ color: THEME.sub, fontWeight: 800 }}>
                {monthLabelFR(selectedMonth)}
              </span>
            </div>
            ...
            >
              <button
                ...
                style={canDownloadSelected ? actionBtnPrimary : actionBtnDisabled}
                disabled={!canDownloadSelected || loading}
              >
              ...
              <button
                ...
                style={canDownloadSelected ? actionBtnGhost : actionBtnDisabled}
                disabled={!canDownloadSelected || loading}
              >
                📗 Télécharger Excel
              </button>
            ...
            {!canDownloadSelected && (
              <div style={hintStyle}>
                Ce mois n’est pas encore validé par l’administration.
              </div>
            )}
          </div>
        </div>
        ...
        >
          <button
            onClick={() => fetchStatuses(selectedYear)}
        ...
        <div style={{ ...cardShell, marginTop: 18 }}>
          ...
          >
            <div>
              ...
              <h2 style={{ margin: "4px 0 0", fontSize: isMobile ? 22 : 26 }}>
                Bordereaux de {selectedYear}
              </h2>
            </div>

            <div style={{ color: THEME.sub, fontWeight: 800 }}>
              Les mois validés apparaissent avec accès direct au PDF et à
              l’Excel.
            </div>
          </div>
                  ...
                  >
                    <div>
                      ...
                      >
                        ...
                        <span
                          style={{
                            color: approved ? THEME.green : THEME.amber,
                          }}
                        >
                          {approved ? "Validé" : "En attente"}
                        </span>
                      </div>
                    </div>
...
const cardShell: CSSProperties = {
  background:
    "linear-gradient(180deg, rgba(17,28,51,0.98) 0%, rgba(14,25,48,0.98) 100%)",
  border: `1px solid ${THEME.border}`,
  borderRadius: 20,
  padding: 16,
};
